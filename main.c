...
{
    printf("usage: %s <serial0> [serial1]\n", program_name);
}
...
{
    const char *serial_name_0 = NULL;
    const char *serial_name_1 = NULL;
    ...
    serial_name_0 = argv[1];
    if (argc == 3)
    {
        serial_name_1 = argv[2];
    }
    ...
    if (serial_name_1 != NULL &&
        !serial_open(serial_name_1, E_SERIAL_BAUD_RATE_115200, 0, E_SERIAL_DATA_BIT_8, E_SERIAL_PARITY_BIT_N,
                     E_SERIAL_STOP_BIT_1))
    {
        printf("open %s fail\n", serial_name_1);
